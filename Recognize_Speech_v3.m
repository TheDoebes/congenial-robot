...
spike_mag = [zero_corr(spike_loc(1)), one_corr(spike_loc(2)), two_corr(spike_loc(3)), three_corr(spike_loc(4))...
    four_corr(spike_loc(5)), five_corr(spike_loc(6)), six_corr(spike_loc(7)), seven_corr(spike_loc(8))...
    eight_corr(spike_loc(9)), nine_corr(spike_loc(10))];
% avg_mag = sum(spike_mag)/10;
%spike_mag is a vector of the magnitude of the spikes on each correlation.
%avg_mag is the average magnitude of the correlation spikes.
...
%%%%%%matchness = spike_mag./centerness; %LOW values of matchness indicate good matches

%match = find(spike_mag == max(spike_mag)) - 1;
...
subplot(3,2,3)
plot(1:len(3),two_ref)
title('Reference "Two" soundbyte')
subplot(3,2,4)
plot(twolags,two_corr)
title('Correlation of input against "Two"')
subplot(3,2,5)
plot(1:len(1),zero_ref)
title('Reference "Zero" soundbyte')
subplot(3,2,6)
plot(zerolags,zero_corr)
 title('Correlation of input against "zero"')
% plot(threelags, three_corr)